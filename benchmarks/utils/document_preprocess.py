...
def load_longdocurl_vlm_text_pages(sample, benchmark_cfg):
    ...
    enriched_pages_path = doc_dir / "vlm_text_pages.json"
    if enriched_pages_path.exists():
        pages = _pages_from_vlm_text_artifact(enriched_pages_path, allowed_pages)
        if pages:
            return pages, allowed_pages
...
def build_token_chunks_from_pages(
    ...
    *,
    ...
    normalized_pages = []
    for page in pages:
        normalized_pages.append({**page, "text": normalize_text_block(page.get("text"))})
...
def colbertv2_query_cache_variant(checkpoint):
    return f"ckpt_{_safe_checkpoint_name(checkpoint)}"
...
def bgem3_query_cache_variant(checkpoint, mode):
    return f"ckpt_{_safe_checkpoint_name(checkpoint)}_mode_{mode}"


def _safe_checkpoint_name(checkpoint):
    checkpoint_name = Path(str(checkpoint)).name or "checkpoint"
    return re.sub(r"[^A-Za-z0-9._-]+", "_", checkpoint_name).strip("._") or "checkpoint"


def _pages_from_vlm_text_artifact(path: Path, allowed_pages):
    with path.open("r", encoding="utf-8") as f:
        payload = json.load(f)
    raw_pages = payload.get("pages") if isinstance(payload, dict) else payload
    if not isinstance(raw_pages, list):
        raise ValueError(f"VLM text artifact must contain a page list: {path}")

    by_index = {}
    for page in raw_pages:
        if not isinstance(page, dict):
            continue
        page_index = int(page.get("page_index", page.get("page_no", -1)))
    ...
    for page_index in allowed_pages:
        ...
        if text:
            pages.append({"page_index": page_index, "page_number": page_index + 1, "text": text})
    return pages
...
def _pages_from_mineru_content_list(content, allowed_pages):
    ...
    for page_index in allowed_pages:
        ...
        if page_text:
            pages.append({"page_index": page_index, "page_number": page_index + 1, "text": page_text})
    return pages
...
def _pages_from_full_md(full_md, allowed_pages):
    ...
    if not text:
        return []
    return [{"page_index": page_index, "page_number": page_index + 1, "text": text} for page_index in allowed_pages]
...
def _build_cross_page_chunks(pages, tokenize_with_spans, chunk_size, chunk_overlap, *, max_cross_pages=None):
    ...
    for chunk_index, start in enumerate(range(0, len(flat_tokens), step)):
        ...
        for token in spans:
            buffer = page_buffers.setdefault(
                token["page_index"],
                {
        ...
        for buffer in page_buffers.values():
