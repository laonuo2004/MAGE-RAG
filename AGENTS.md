...
- 在进行诸如代码修改、论文撰写登相关工作时，**不要考虑任何向后兼容性问题**！我们只允许最新的标准和最佳实践，过时的内容不允许存在于代码库中，如果存在没有更新的部分，不应该写一个向后兼容的处理，也不应该在这里刻意抛出一个报错，提示没有更新。最佳实践是让没有更新的部分**自然报错**，后续我们根据报错去一一修复问题，使得整个代码浑然一体，像是**一次性完成**的工作。这样做是为了以免引发代码库混乱、误导后续的开发者。
