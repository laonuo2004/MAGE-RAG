...
`*.metrics.json`；Hydra 运行配置和日志保存在 `outputs/`。

# 引用

如果本仓库对你的研究有帮助，请引用 [MAGE-RAG](https://arxiv.org/abs/2606.15906)。
...
- MinerU：提供 PDF 文档解析服务 (https://mineru.net/)
- vLLM：用于模型部署与推理 (https://vllm.ai/)
- Qwen3-VL-8B-Instruct：用于 Reader/Controller 模型 (https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct)
